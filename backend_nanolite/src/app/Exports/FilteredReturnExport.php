<?php

namespace App\Exports;

use App\Models\ProductReturn;
use Maatwebsite\Excel\Concerns\FromArray;
use Maatwebsite\Excel\Concerns\WithStyles;
use PhpOffice\PhpSpreadsheet\Style\Alignment;
use PhpOffice\PhpSpreadsheet\Style\Border;
use PhpOffice\PhpSpreadsheet\Style\Fill;
use PhpOffice\PhpSpreadsheet\Worksheet\Worksheet;

class FilteredReturnExport implements FromArray, WithStyles
{
    protected array $filters;

    public function __construct(array $filters = [])
    {
        $this->filters = $filters;
    }

    public function array(): array
    {
        $query = ProductReturn::with([
            'customer.customerCategory',
            'employee',
            'category',
        ]);

        if (!empty($this->filters['department_id'])) {
            $query->where('department_id', $this->filters['department_id']);
        }
        if (!empty($this->filters['customer_id'])) {
            $query->where('customer_id', $this->filters['customer_id']);
        }
        if (!empty($this->filters['employee_id'])) {
            $query->where('employee_id', $this->filters['employee_id']);
        }
        if (!empty($this->filters['customer_categories_id'])) {
            $query->where('customer_categories_id', $this->filters['customer_categories_id']);
        }
        if (!empty($this->filters['status'])) {
            $query->where('status', $this->filters['status']);
        }

        $returns = $query->get();

        // Filter manual berdasarkan brand, kategori, produk
        if (!empty($this->filters['brand_id'])) {
            $returns = $returns->filter(function ($return) {
                foreach ($return->productsWithDetails() as $item) {
                    if (($item['brand_id'] ?? null) == $this->filters['brand_id']) {
                        return true;
                    }
                }
                return false;
            });
        }

        if (!empty($this->filters['category_id'])) {
            $returns = $returns->filter(function ($return) {
                foreach ($return->productsWithDetails() as $item) {
                    if (($item['category_id'] ?? null) == $this->filters['category_id']) {
                        return true;
                    }
                }
                return false;
            });
        }

        if (!empty($this->filters['product_id'])) {
            $returns = $returns->filter(function ($return) {
                foreach ($return->productsWithDetails() as $item) {
                    if (($item['product_id'] ?? null) == $this->filters['product_id']) {
                        return true;
                    }
                }
                return false;
            });
        }

        // Header dan judul
        $rows = [
            ['', '', '', '', '', '', '', '', '', 'PRODUCT RETURN', '', '', '', '', ''],
            [
                'No.', 'No Return', 'Tanggal Dibuat', 'Tanggal Diupdated', 'Department', 'Karyawan', 'Customer', 'Kategori Customer',
                'Phone', 'Alamat', 'Item', 'Jumlah', 'Nominal', 'Alasan', 'Catatan Tambahan', 'Status'
            ],
        ];

        $no = 1;
        foreach ($returns as $return) {
            $itemDetails = $return->productsWithDetails();
            $deskripsi = collect($itemDetails)->map(function ($item) {
                return "{$item['brand_name']} – {$item['category_name']} – {$item['product_name']} – {$item['color']}";
            })->implode("\n");

            $totalQty = collect($itemDetails)->sum('quantity');

            $rows[] = [
                $no++,
                $return->no_return ?? '-',
                optional($return->created_at)->format('Y-m-d H:i'),
                optional($return->updated_at)->format('Y-m-d H:i'),
                $return->department->name ?? '-',
                $return->employee->name ?? '-',
                $return->customer->name ?? '-',
                $return->category->name ?? '-',
                $return->phone ?? '-',
                is_array($return->address) ? implode(', ', $return->address) : $return->address,
                $deskripsi,
                $totalQty,
                'Rp ' . number_format($return->amount, 0, ',', '.'),
                $return->reason ?? '-',
                $return->note ?? '-',
                ucfirst($return->status),
            ];
        }

        return $rows;
    }

    public function styles(Worksheet $sheet)
    {
        $sheet->getStyle('J1')->applyFromArray([
            'font' => ['bold' => true, 'size' => 14],
            'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER],
        ]);

        $sheet->getStyle('A2:P2')->applyFromArray([
            'font' => ['bold' => true],
            'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER],
            'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'E8EAF6']],
            'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN]],
        ]);

        $highestRow = $sheet->getHighestRow();

        foreach (range(3, $highestRow) as $row) {
            foreach (range('A', 'P') as $col) {
                $sheet->getStyle("{$col}{$row}")->applyFromArray([
                    'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN]],
                    'alignment' => [
                        'horizontal' => Alignment::HORIZONTAL_CENTER,
                        'vertical' => Alignment::VERTICAL_TOP,
                        'wrapText' => true,
                    ],
                ]);
            }
        }

        foreach (range('A', 'P') as $col) {
            $sheet->getColumnDimension($col)->setAutoSize(true);
        }

        return [];
    }
}
